...
const Body = () => {
  ...
  const handleAdd = async () => {
    ...
    else addCardMessage(message + `(${card.name}, ${card.subject}, ${card.score})`);
  };

  const handleQuery = async () => {
    const {
      data: { messages, message },
    } = await axios.get('/api/query-card', {params:{queryType:queryType, queryString:queryString}}) // TODO: axios.xxx call the right api
